...
namespace RothurMod.Items.ExampleDamageClass
{
	...
	public abstract class ExampleDamageItem : ModItem
	{
		...
		public override void ModifyWeaponDamage(Player player, ref float add, ref float mult, ref float flat) {
			add += ExampleDamagePlayer.ModPlayer(player).NecroDamageAdd;
			mult *= ExampleDamagePlayer.ModPlayer(player).NecroDamageMult;
		}

		public override void GetWeaponKnockback(Player player, ref float knockback) {
			// Adds knockback bonuses
			knockback += ExampleDamagePlayer.ModPlayer(player).NecroKnockback;
		}

		public override void GetWeaponCrit(Player player, ref int crit) {
			// Adds crit bonuses
			crit += ExampleDamagePlayer.ModPlayer(player).NecroCrit;
		}
		...
		public override void ModifyTooltips(List<TooltipLine> tooltips) {
			...
			if (tt != null) {
				...
				tt.text = damageValue + " necromantic " + damageWord;
			}
