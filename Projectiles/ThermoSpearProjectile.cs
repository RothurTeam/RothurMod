...
{
	public class ThermoSpearProjectile : ModProjectile
	{
		...
		public override void Kill(int timeLeft) {
			...
			if (projectile.owner == Main.myPlayer) {
				...
				Main.rand.NextBool(18)
					? Item.NewItem(projectile.getRect(), ItemType<ThermoSpear>())
					: 0;
		...
		public override void ModifyHitNPC(NPC target, ref int damage, ref float knockback, ref bool crit, ref int hitDirection) {
			...
			projectile.damage = 1; // Makes sure the sticking javelins do not deal damage anymore
		...
		private void UpdateStickyJavelins(NPC target)
		{
			...
			for (int i = 0; i < Main.maxProjectiles; i++) // Loop all projectiles
			{
				...
				if (i != projectile.whoAmI // Make sure the looped projectile is not the current javelin
				    ...
				    && currentProjectile.modProjectile is RealmSpearProjectile javelinProjectile 
		...
		private void NormalAI()
		{
			...
			if (Main.rand.NextBool(3)) {
				Dust dust = Dust.NewDustDirect(projectile.position, projectile.height, projectile.width, DustType<SparkleR>(),
