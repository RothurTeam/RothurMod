...
namespace RothurMod.Projectiles
{
	...
	public class MagicMissile: ModProjectile
	{
		...
		public override void AI() {
			...
			Dust dust = Dust.NewDustPerfect(dustPosition, ModContent.DustType<BloodyFlame> (), null, 100, Color.Lime, 0.5f);
			dust.velocity *= 0.4f;
			dust.noGravity = true;
		...
		public override void Kill(int timeLeft) {
			...
			for (int i = 0; i < 15; i++) {
				Dust dust = Dust.NewDustDirect(projectile.position - projectile.velocity, projectile.width, projectile.height, ModContent.DustType<BloodyFlame>(), 0, 0, 100, Color.Lime, 0.8f);
				dust.noGravity = true;
				dust.velocity *= 2f;
				dust = Dust.NewDustDirect(projectile.position - projectile.velocity, projectile.width, projectile.height, ModContent.DustType<BloodyFlame>(), 0f, 0f, 100, Color.Lime, 0.5f);
			}
		}
	}
}