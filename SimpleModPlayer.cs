...
namespace RothurMod
{
	...
	public class SimpleModPlayer : ModPlayer
	{
		...
		public override void OnHitNPCWithProj(Projectile proj, NPC target, int damage, float knockback, bool crit) {
			...
			if ((proj.minion || ProjectileID.Sets.MinionShot[proj.type]) && FrostBurnSummon && !proj.noEnchantments) {
				...
				target.AddBuff(BuffID.Frostburn, 60 * Main.rand.Next(5, 12), false);
			}
		}
	    ...
	    namespace Items
	{
		...
		{
			DisplayName.AddTranslation(GameCulture.Russian, "Каменное сердце");
		}
		...
		internal class IceHeart : ModItem
		{
			public override void SetStaticDefaults() 
		{
			DisplayName.AddTranslation(GameCulture.Russian, "Ледяное сердце");
			Tooltip.SetDefault("Summonable creatures cast an ice burn on their enemies");
			Tooltip.AddTranslation(GameCulture.Russian, "Призываемые существа накладывают на врагов ледяной ожог");
		}
		    ...
		    {
			ModRecipe recipe = new ModRecipe(mod);
			recipe.AddIngredient(null, "StoneHeart", 1);
